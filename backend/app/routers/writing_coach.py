...
def create_submission(
    ...
    return _detail(writing_coach.create_and_evaluate(db, payload, provider))
...
def list_submissions(db: Session = Depends(get_db)):
    return [
        ...
        for value in writing_coach.get_submission_list(db)
    ]
...
def submission_detail(
    ...
    value = writing_coach.get_submission_detail(db, submission_id)
    if value is None:
